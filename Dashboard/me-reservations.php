...
if(!isset($_SESSION["info"])){
    ...
    $sqlClient = "SELECT * FROM users WHERE email = '".  $userInfo["email"] . "'";
    $resClient = mysqli_query($conn,$sqlClient);
    $clientFetch = mysqli_fetch_assoc($resClient);
    $sqlRes = "SELECT r.id,adresse,r.etat,présence_prop,is_payed,heure_res,date_res FROM reservation r , location l  WHERE l.id = r.location_id AND  client_id = ". $clientFetch["id"];
